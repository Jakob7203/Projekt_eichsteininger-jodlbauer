...
public class Utils {
    ...
    public static Download getDownload(String url) throws IOException {
        ...
        String downloadUrl = pageSource.select(DOWNLOAD_BTN).first().attr(DOWNLOAD_LINK);//click buttons on site
        String songTitle = pageSource.select(PARAGRAPH).get(SONG_TITLE).text();//maybe try this yourself with
